...
class StatisticsManager:
    ...
    def show_statistics(self):
        ...
        # Botões laterais (barras, motivos, agências, + acumulado)
    ...
    def _apply_period_filter(self, df):
        ...
        aplicando df.copy() após cada filtragem para evitar
        ...
        if self.current_period == "mes":
            ...
            def year_sem(dt):
                ...
                return (y, s)
            df["YearSem"] = df["Carimbo de data/hora"].apply(year_sem)
            return df
    ...
    def update_info_box(self, df):
        if df.empty:
            ...
            # Como só há Status=Pago, pending=0 e awaiting=0
            pending_requests = 0
            awaiting_payment_requests = 0
            ...
            if self.current_period == "mes":
                ...
                if "SemanaMes" not in df.columns:
                    ...
                    if pivot.empty:
                        ...
                        ax.set_xticks(x)
                        ax.set_xticklabels([f"Sem{i}" for i in weeks_index])
